...
CTermCharAttr::m_DefaultColorTable[SIZE_OF_COLOR_TABLE] = {
	//Darker color
	RGB(0,0,0), 		//0;30m		Black
	RGB(128,0,0),		//0;31m		Dark red
	RGB(0,128,0),		//0;32m		Dark green
	RGB(128,128,0),	//0;33m		Brown
	RGB(0,0,128),		//0;34m		Dark blue
	RGB(128,0,128),	//0;35m		Dark magenta
	RGB(0,128,128),	//0;36m		Dark cyan
	RGB(192,192,192),	//0;37m		Light gray
	//Bright color
	RGB(128,128,128),	//1;30m		Gray
	RGB(255,0,0),		//1;31m		Red
	RGB(0,255,0),		//1;32m		Green
	RGB(255,255,0),	//1;33m		Yellow
	RGB(0,0,255),		//1;34m		Blue
	RGB(255,0,255),	//1;35m		Magenta
	RGB(0,255,255),	//1;36m		Cyan
	RGB(255,255,255)	//1;37m		White
};
...
void CTermData::UpdateDisplay()
{
	...
	CTermData* pTermData = m_pView->m_pTermData;
	if( pTermData == this )
	{
		...
		HDC dc = GetDC(m_pView->m_hWnd);
//		m_pView->PrepareDC(dc);

		HGDIOBJ oldfnt = SelectObject( dc, m_pView->m_Font );
			...
			for( ; col < m_ColsPerPage; col++  )
			{
				if( attr[col].IsNeedUpdate() )
				{
					if( col>0 && attr[col].GetCharSet()==CTermCharAttr::CS_MBCS2 )
						col--;
					m_pView->DrawChar( dc, line, col, top );
...
string CTermData::GetText(POINT start, POINT end, bool trim, bool block)
{
	string text;
//	Use system specific design here.
	...
	if( start.y == end.y )
	{
		text = string( m_Screen[start.y]+start.x, end.x-start.x );
//		text.Trim();
		str_striptail( text );
		return text;
	}
	...
	if( block )
	{
		...
		{
			text += string( m_Screen[iline]+start.x, (end.x-start.x) );
//			text.Trim();
			str_striptail( text );
			text += eol;
		}
	...
	{
		text = string( m_Screen[start.y]+start.x, m_ColsPerPage-1-start.x );
//		text.Trim();
		str_striptail( text );
		text += eol;
...
//		text.Trim();	// triim all space at tail.
...
// 2004.08.27 modified by PCMan
// if trim == true, excess line at tail will be removed.
string CTermData::GetTextWithColor(POINT start, POINT end, bool trim, bool block)
{
//	Use system specific design here.
#ifdef	_WINDOWS
	char eol[] = "\r\n";
...
void CTermData::SetTextAttr( CTermCharAttr attr, int flags, POINT start, POINT end, bool block)
{
