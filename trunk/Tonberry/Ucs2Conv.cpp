...
void CUcs2Conv::InitBig52Ucs2Tab()
{
    /*HANDLE hFile = CreateFile("C:\\B2U",          // file to open
    ...
    if(!ReadFile(hFile, m_Big52Ucs2_Tab, size, &dwBytesRead, NULL))
    {
	m_Big52Ucs2_Tab = NULL;
	CloseHandle(hFile);
        return false;
...
void CUcs2Conv::InitUcs22Big5Tab()
{
    /*HANDLE hFile = CreateFile("C:\\U2B",          // file to open
                       GENERIC_READ, FILE_SHARE_READ, NULL,
					   OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	m_Ucs2TabSize = GetFileSize(hFile, NULL);
	m_Ucs22Big5_Tab = new unsigned char[m_Ucs2TabSize];
	unsigned long dwBytesRead = 0;

    if(!ReadFile(hFile, m_Ucs22Big5_Tab, m_Ucs2TabSize, &dwBytesRead, NULL))
    {
	m_Ucs2TabSize = 0;
	m_Ucs22Big5_Tab = NULL;
	CloseHandle(hFile);
	...
	m_Ucs2TabSize = ( sizeof(m_Ucs22Big5_Tab) / sizeof(m_Ucs22Big5_Tab[0]) );
}
